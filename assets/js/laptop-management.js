...
function getLaptopsData() {
    ...
    if(searchValue != "") {
        const filteredLaptops = laptops.filter(laptop =>
            laptop.name.trim().toUpperCase().includes(searchValue)
        );
...
function setImageLink(button) {
    ...
    if (selectedFiles.length > 0) {
        ...
        link = "http://127.0.0.1/" + selectedFile.name;
...
function showAddProduct() {
    ...
    //         <label for="product-band">Hãng:</label>
    //         <select id="product-band">
    //             <option value="hp" ng-repeat="band in listDanhMuc">HP</option>
    ...
    //         <label for="product-new-price">Giá:</label>
    //         <input type="text" oninput="formatInputNumber(this)" id="product-new-price" placeholder="100,000,000">
    //         <label for="product-quantity">Số lượng:</label>
    //         <input type="text" oninput="formatInputNumber(this)" id="product-quantity" placeholder="10,000">
    //         <div class="detail-specs">
    //             <h3>Thông tin cấu hình <i class="fa-solid fa-plus" onclick="addLaptopSpec()"></i></h3>
    //         </div>
    //         <div class="detail-nav">
...
function addLaptopSpec() {
    // Tạo các phần tử input mới
    let inputSpecName = document.createElement("input");
    inputSpecName.type = "text";
    inputSpecName.className = "spec-name";
    inputSpecName.placeholder = "Tên thông số";
    inputSpecName.style.marginTop = "10px";

    let inputSpecDesc = document.createElement("input");
    inputSpecDesc.type = "text";
    inputSpecDesc.className = "spec-desc";
    inputSpecDesc.placeholder = "Mô tả";

    // Lấy đối tượng .detail-specs từ DOM
    let detailSpecsElement = document.querySelector(".detail-specs");

    // Thêm các phần tử input vào .detail-specs
    detailSpecsElement.appendChild(inputSpecName);
    detailSpecsElement.appendChild(inputSpecDesc);
}
